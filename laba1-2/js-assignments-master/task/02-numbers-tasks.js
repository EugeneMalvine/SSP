...
function getLinearEquationRoot(a, b) {
    ...
    return (-1* b)/a;
    throw new Error('Not implemented');
    // а тут всё работает , даже деление на ноль; 
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    var act1=x1*x2 + y1*y2;
    var act2=Math.hypot(x1, y1);
    var act3=Math.hypot(x2, y2);
    var res = act1/(act2*act3);
...
function roundToPowerOfTen(num, pow) {
    var act1=Math.pow(10, pow);
    var act2= Math.round(num/act1);
    return act2*act1;
    throw new Error('Not implemented');
}
...
function isPrime(n) {
    ...
    return res;
    throw new Error('Not implemented');
    // почему не работает вместе с else ?
}
...
function toNumber(value, def) {
    ...
    if(typeof tmp === 'number' && isFinite(tmp))
        return value;
    else
        return def;
    throw new Error('Not implemented');
